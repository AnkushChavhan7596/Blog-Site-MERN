...
const Comments = ({post}) => {
  ...
  const handleComment = async (e) =>{
    e.preventDefault();
     ...
     try{
       const res = await axios.post(`/api/comments/comment/${activeUser?._id}`,{postId : post._id, comment : comment});
      ...
      <div className="comment_form_wrapper">
        <form onSubmit={handleComment}>
          <div className="input_field">
            <textarea type="text" value={comment} onChange={(e)=>{setComment(e.target.value)}} placeholder='Enter Comment .....' />
          </div>
      ...
      <div className="comments_wrapper">
        {
          ...
          //   return <Comment comment={comment} key={comment._id} />
            ...
            comments?.filter((comment) => comment.postId === post?._id).reverse().map((comment)=>{
              return <Comment comment={comment} key={comment._id}/>
            })
        }
      </div>
